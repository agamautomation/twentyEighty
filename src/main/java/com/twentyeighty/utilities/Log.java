...
public class Log {
	private int num = 1;
	private int step = 1;
	private static final String directory = "D:\\Lovekesh\\";
	private WritableWorkbook workLog;
	private WritableSheet workSheet;
	private WritableFont font;
	private WritableCellFormat fontColor;
	private File fileXLS;  //log file
	...
	private void Log(String name, String directory2) {
		// TODO Auto-generated method stub
	...
	public Log(String name, String path) {
		...
		fileXLS = new File(path + name + ".xls");
		...
		*change the aligment.
		...
		try {
			...
			fontColor.setBackground(Colour.BLUE);
		...
		*for the column the first argument is the column number the second is the height which is 1 to 1 
		...
		try {
			...
			label = new Label(1, 0, "Step Description",fontColor); 
			workSheet.addCell(label);
			
			label = new Label(2, 0, "Test Behavour",fontColor); 
			workSheet.addCell(label);
	 ...
	 */
	public void write(String description, String stepName ,Boolean Passed) throws IOException, RowsExceededException, WriteException
	{
		...
		label = new Label(3, num, result, fontColor);
		workSheet.addCell(label);
...
//		}
	...
	public void takeSnapshot(String filePath, String fileName)
	{
		...
		WritableImage writeableimage = new WritableImage(1, step, 2, 30, imgFile);
		workSheet.addImage(writeableimage);
		num = num + 30;
